...
import type { TaskGenerationTrace } from "@/lib/content-creation-types";
...
describe("WorkspaceShell", () => {
  ...
  it("adds a new platform from the workspace and generates content immediately", async () => {
    const user = userEvent.setup();
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      if (String(input) === "/api/tasks/task-1/regenerate" && init?.method === "POST") {
        return {
    ...
    await user.click(screen.getByRole("button", { name: "生成小红书笔记" }));
  ...
  it("generates all remaining platforms in one click", async () => {
    const user = userEvent.setup();
    const fetchMock = vi
      .fn()
    ...
    await user.click(screen.getByRole("button", { name: "一键全生成" }));
