...
const expandablePlatforms: PlatformId[] = ["xiaohongshu", "twitter", "videoScript"];
...
export function WorkspaceShell(props: {
  ...
  const [isGeneratingAllExpansionPlatforms, setIsGeneratingAllExpansionPlatforms] =
    useState(false);
  ...
  const availableExpansionPlatforms = expandablePlatforms.filter(
    (platform) => !task.selectedPlatforms.includes(platform)
  );
  const isAnyGenerating = isRegenerating || isGeneratingAllExpansionPlatforms;
  const publishLabel =
  ...
  async function handleRegenerate(options?: {
    ...
    setStatusText(
      includePlatform
        ...
        : `重新生成 ${platformLabels[targetPlatform]} 中...`
    );
    ...
    try {
      ...
      if (includePlatform) {
        regenerateBody.includePlatform = true;
      }

      if (targetPlatform === "twitter" && bundle.twitter) {
      ...
      } else if (!result.bundle[activePlatform]) {
        const nextPlatform =
      ...
      setStatusText(includePlatform ? "新增平台内容生成完成" : "重新生成完成");
      return true;
    } catch (error) {
      setStatusText(
  ...
  async function handleGenerateAllExpansionPlatforms() {
    const targets = expandablePlatforms.filter(
      (platform) => !task.selectedPlatforms.includes(platform)
    );

    if (targets.length === 0) {
      setStatusText("当前没有可新增的平台内容。");
      return;
    }

    setIsGeneratingAllExpansionPlatforms(true);
    let successCount = 0;
    let failedCount = 0;
    ...
    try {
      ...
      setStatusText(
        failedCount === 0
          ? `一键全生成完成：${successCount} 个平台已生成`
          : `一键全生成完成：成功 ${successCount}，失败 ${failedCount}`
      );
    } finally {
      setIsGeneratingAllExpansionPlatforms(false);
    }
  }
    ...
    <main className="workspace-layout">
      ...
      <section className="workspace-canvas">
        ...
        <div className="workspace-card">
          ...
          {availableExpansionPlatforms.length > 0 ? (
            <div className="workspace-card__expansion">
              <p className="workspace-card__expansion-title">
                还可生成其他平台内容
              </p>
              <div className="workspace-card__expansion-actions">
                {availableExpansionPlatforms.length > 1 ? (
                  <button
                    className="workspace-card__expansion-button workspace-card__expansion-button--primary"
                    type="button"
                    disabled={isAnyGenerating}
                    onClick={() => {
                      void handleGenerateAllExpansionPlatforms();
                    }}
                  >
                    {isGeneratingAllExpansionPlatforms ? "一键全生成中..." : "一键全生成"}
                  </button>
                ) : null}
                {availableExpansionPlatforms.map((platform) => (
                  <button
                    key={platform}
                    className="workspace-card__expansion-button"
                    type="button"
                    disabled={isAnyGenerating}
                    onClick={() => {
                  ...
                  >
                    {generatingPlatform === platform
                      ? `生成${platformLabels[platform]}中...`
                      : `生成${platformLabels[platform]}`}
                  </button>
                ))}
              </div>
            </div>
            ...
            quickFixAction={
              showExpiredImageQuickFix
                ? {
                    ...
                    disabled: isPublishing || isAnyGenerating
                  }
                : undefined
            }
