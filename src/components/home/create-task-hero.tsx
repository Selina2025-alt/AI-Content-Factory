...
import type { DraftRecord, PlatformId, WechatLibraryItem } from "@/lib/content-creation-types";
...
export function CreateTaskHero() {
  ...
  const hasGeneratingDraft = drafts.some((draft) => draft.status === "generating");
  ...
  }, [hasGeneratingDraft, refreshDrafts]);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      startTransition(() => {
      router.push(`/content-creation/workspace/${task.id}`);
      });
    } catch (error) {
      setFormError(
  ...
  function handleContinueDraft(draft: DraftRecord) {
    if (draft.status === "generated" && draft.lastGeneratedTaskId) {
      startTransition(() => {
      router.push(`/content-creation/workspace/${draft.lastGeneratedTaskId}`);
      });
      return;
    }
