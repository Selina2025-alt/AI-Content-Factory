...
import {
  ReplicaTopicLibraryPanel,
  type TopicLibraryBatchResult
} from "@/components/workbench/replica-topic-library-panel";
...
import {
  ...
  type ReplicaTopicLibraryEntry,
  type ReplicaTopicSuggestion,
...
interface TopicLibraryListResponsePayload {
  items?: ReplicaTopicLibraryEntry[];
  error?: string;
}

interface TopicLibraryBatchGenerateResultItem {
  entryId: string;
  title: string;
  success: boolean;
...
interface TopicLibraryBatchGenerateResponsePayload {
  total?: number;
  successCount?: number;
  failedCount?: number;
  message?: string;
  results?: TopicLibraryBatchGenerateResultItem[];
}
...
interface PersistedMonitoringCategoriesState {
...
function isPersistedCategoryCandidate(value: unknown): value is ReplicaCategory {
  ...
  const candidate = value as Partial<ReplicaCategory>;

  return (
    typeof candidate.id === "string" &&
    typeof candidate.name === "string" &&
    typeof candidate.keyword === "string" &&
    Array.isArray(candidate.keywordTargets) &&
...
export function MonitoringWorkbench() {
  ...
  const [topicLibraryEntries, setTopicLibraryEntries] = useState<ReplicaTopicLibraryEntry[]>([]);
  const [topicLibraryLoading, setTopicLibraryLoading] = useState(false);
  const [topicLibraryStatusMessage, setTopicLibraryStatusMessage] = useState("");
  const [topicLibraryBatchResults, setTopicLibraryBatchResults] = useState<
    TopicLibraryBatchResult[]
  >([]);
  const [topicLibraryGenerating, setTopicLibraryGenerating] = useState(false);
  const [topicLibraryUpsertingTopicIds, setTopicLibraryUpsertingTopicIds] = useState<string[]>([]);
  ...
  const activeTopicLibraryEntries = useMemo(
    () => topicLibraryEntries.filter((entry) => !entry.isDeleted),
    [topicLibraryEntries]
  );
  const topicLibraryTopicIds = useMemo(
    () => activeTopicLibraryEntries.map((entry) => entry.sourceTopicId),
    [activeTopicLibraryEntries]
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      const parsed = JSON.parse(rawValue) as Partial<PersistedMonitoringCategoriesState>;
      const persistedCategories = Array.isArray(parsed.categories)
        ? parsed.categories.filter(isPersistedCategoryCandidate)
        : [];
      ...
      const preferredActiveCategoryId =
        typeof parsed.activeCategoryId === "string" ? parsed.activeCategoryId : "";
      const nextActiveCategoryId = persistedCategories.some(
        (category) => category.id === preferredActiveCategoryId
      )
        ? preferredActiveCategoryId
  ...
  useEffect(() => {
    void loadTopicLibrary();
  }, []);

  useEffect(() => {
    if (activeTab !== "topicLibrary") {
      return;
    }

    void loadTopicLibrary({ quiet: true });
  }, [activeTab]);
  ...
  useEffect(() => {
    ...
    const hasRunningJobs = topicLibraryEntries.some(
      (entry) => !entry.isDeleted && entry.generationStatus === "generating"
    );
  ...
  function buildTopicLibraryEntryId(topicId: string) {
    return `topic-library:${topicId}`;
  }

  async function loadTopicLibrary(options?: { quiet?: boolean }) {
    if (typeof fetch !== "function") {
      return;
    }

    if (!options?.quiet) {
      setTopicLibraryLoading(true);
    }
    ...
    try {
      ...
      const payload = (await response.json()) as TopicLibraryListResponsePayload;

      if (!response.ok) {
        throw new Error(payload.error ?? "Failed to load topic library");
      }

      setTopicLibraryEntries(payload.items ?? []);
    } catch (error) {
      setTopicLibraryStatusMessage(
        error instanceof Error ? error.message : "Failed to load topic library"
      );
    } finally {
      if (!options?.quiet) {
        setTopicLibraryLoading(false);
      }
    }
  }

  async function patchTopicLibrary(input: {
    ids: string[];
    selected?: boolean;
    isDeleted?: boolean;
  }) {
    if (typeof fetch !== "function" || input.ids.length === 0) {
      return;
    }
    ...
    if (!response.ok) {
      throw new Error(payload.error ?? "Failed to update topic library");
    }

    setTopicLibraryEntries(payload.items ?? []);
  }

  async function handleToggleTopicLibrary(input: {
    ...
    setTopicLibraryStatusMessage("");
    setTopicLibraryUpsertingTopicIds((current) =>
      current.includes(input.topic.id) ? current : [...current, input.topic.id]
    );
      ...
      if (input.checked) {
        const response = await fetch("/api/topic-library", {
          ...
          body: JSON.stringify({
            ...
            keyword: activeKeywordTargetKeyword || activeKeyword || activeCategory.keyword,
            reportDate: input.reportDate,
            sourceSnapshotId: input.reportId,
            topics: [
              {
                id: input.topic.id,
                title: input.topic.title,
                intro: input.topic.intro,
                whyNow: input.topic.whyNow,
                hook: input.topic.hook,
                growth: input.topic.growth,
                supportContentIds: input.topic.supportContentIds
              }
            ]
          })
        });
        ...
        if (!response.ok) {
          throw new Error(payload.error ?? "Failed to add topic to library");
        }

        setTopicLibraryEntries(payload.items ?? []);
        setTopicLibraryStatusMessage("Topic added to library");
      } else {
        await patchTopicLibrary({
          ids: [buildTopicLibraryEntryId(input.topic.id)],
          isDeleted: true
        });
        setTopicLibraryStatusMessage("Topic removed from library");
      }
    } catch (error) {
      setTopicLibraryStatusMessage(
        error instanceof Error ? error.message : "Failed to update topic library"
      );
    } finally {
      setTopicLibraryUpsertingTopicIds((current) =>
        current.filter((topicId) => topicId !== input.topic.id)
      );
    }
  }

  function handleOpenTopicLibrary() {
    setActiveTab("topicLibrary");
    void loadTopicLibrary({ quiet: true });
  }

  function handleToggleTopicLibrarySelection(entryId: string, selected: boolean) {
    setTopicLibraryEntries((current) =>
      current.map((entry) => (entry.id === entryId ? { ...entry, selected } : entry))
    );
    void patchTopicLibrary({
      ids: [entryId],
      selected
    }).catch((error) => {
      setTopicLibraryStatusMessage(
  ...
  function handleToggleTopicLibrarySelectionAll(selected: boolean) {
    const ids = activeTopicLibraryEntries.map((entry) => entry.id);
    ...
    setTopicLibraryEntries((current) =>
      current.map((entry) => (ids.includes(entry.id) ? { ...entry, selected } : entry))
    );
    void patchTopicLibrary({
      ids,
      selected
  ...
  function handleSoftDeleteTopicLibraryEntry(entryId: string) {
    setTopicLibraryEntries((current) =>
      current.map((entry) =>
        entry.id === entryId ? { ...entry, isDeleted: true } : entry
      )
    );
    void patchTopicLibrary({
      ids: [entryId],
  ...
  async function handleBatchGenerateTopicLibraryEntries() {
    const selectedEntries = activeTopicLibraryEntries.filter((entry) => entry.selected);

    if (selectedEntries.length === 0) {
      setTopicLibraryStatusMessage("Please select topics before batch generation");
      return;
    }
    ...
    setTopicLibraryGenerating(true);
    setTopicLibraryStatusMessage("");
    setTopicLibraryBatchResults([]);
    ...
    try {
      const response = await fetch("/api/topic-library/batch-generate", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({
          ...
          autoGenerateWechatCover: true,
          enableWebSearch: true
        })
      });
      const payload = (await response.json()) as TopicLibraryBatchGenerateResponsePayload;

      if (!response.ok) {
        throw new Error(payload.message ?? "Batch generation failed");
      }
      ...
      setTopicLibraryStatusMessage(payload.message ?? "Batch generation started in background");
      await loadTopicLibrary({ quiet: true });
    } catch (error) {
      setTopicLibraryStatusMessage(
        error instanceof Error ? error.message : "Batch generation failed"
      );
    } finally {
      setTopicLibraryGenerating(false);
    }
  }
          ...
          <section className="replica-shell__workspace-body">
            ...
            {activeTab === "analysis" ? (
              <ReplicaAnalysisPanel
                ...
                topicLibraryTopicIds={topicLibraryTopicIds}
                upsertingTopicIds={topicLibraryUpsertingTopicIds}
                ...
                onOpenTopicLibrary={handleOpenTopicLibrary}
              />
            ) : null}

            {activeTab === "topicLibrary" ? (
              <ReplicaTopicLibraryPanel
                entries={activeTopicLibraryEntries}
                isLoading={topicLibraryLoading}
                isGenerating={topicLibraryGenerating}
                statusMessage={topicLibraryStatusMessage}
                results={topicLibraryBatchResults}
                onToggleSelect={handleToggleTopicLibrarySelection}
                onToggleSelectAll={handleToggleTopicLibrarySelectionAll}
                onSoftDelete={handleSoftDeleteTopicLibraryEntry}
                onBatchGenerate={() => void handleBatchGenerateTopicLibraryEntries()}
              />
            ) : null}
