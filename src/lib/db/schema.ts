...
export function ensureMonitoringSchema(database: MonitoringDatabase) {
  database.exec(`
    ...
    CREATE TABLE IF NOT EXISTS topic_library_entries (
      id TEXT PRIMARY KEY,
      source_topic_id TEXT NOT NULL UNIQUE,
      ...
      hook TEXT NOT NULL,
      growth TEXT NOT NULL,
      support_content_ids_json TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS topic_library_entries_deleted_idx
      ON topic_library_entries (is_deleted, updated_at DESC);

    CREATE INDEX IF NOT EXISTS topic_library_entries_selected_idx
      ON topic_library_entries (selected, is_deleted, updated_at DESC);
