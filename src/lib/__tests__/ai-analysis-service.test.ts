...
describe("ai analysis service", () => {
  it("returns structured evidence items and backfills to eight diversified topic suggestions", async () => {
    ...
    expect(result?.snapshot.topics.length).toBe(8);
    expect(
      new Set(result?.snapshot.topics.map((topic) => topic.title)).size
    ).toBe(8);
  });
});
