...
describe("siliconflow client", () => {
  ...
  it("uses the default image model when image model env is missing", () => {
    process.env.SILICONFLOW_API_KEY = "test-key";
    delete process.env.SILICONFLOW_IMAGE_MODEL;

    expect(getSiliconFlowImageConfig()?.model).toBe("Qwen/Qwen-Image-Edit-2509");
  });
  ...
  it("falls back to the next candidate model when the current model is unavailable", async () => {
    process.env.SILICONFLOW_API_KEY = "test-key";
    process.env.SILICONFLOW_BASE_URL = "https://api.siliconflow.cn/v1";
    process.env.SILICONFLOW_IMAGE_MODEL = "Qwen/Qwen-Image-Edit-2509";
    ...
    expect(firstBody.model).toBe("Qwen/Qwen-Image-Edit-2509");
  ...
  it("switches model when current model reports it does not support image input", async () => {
    process.env.SILICONFLOW_API_KEY = "test-key";
    process.env.SILICONFLOW_BASE_URL = "https://api.siliconflow.cn/v1";
    process.env.SILICONFLOW_IMAGE_MODEL = "Qwen/Qwen-Image";
    process.env.SILICONFLOW_IMAGE_MODEL_FALLBACKS = "Qwen/Qwen-Image-Edit-2509";
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ...
        text: async () =>
          JSON.stringify({
            code: 40001,
            message: "This model does not support image input parameter"
          })
      })
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          data: [
            {
              url: "https://cdn.example.com/image-edit-success.png"
            }
          ]
        })
      });
    ...
    await expect(
      createSiliconFlowImageGeneration({
        prompt: "Generate a polished cover",
        image: "data:image/png;base64,AAA"
      })
    ).resolves.toBe("https://cdn.example.com/image-edit-success.png");

    const firstBody = JSON.parse(String(fetchMock.mock.calls[0]?.[1]?.body));
    const secondBody = JSON.parse(String(fetchMock.mock.calls[1]?.[1]?.body));

    expect(firstBody.model).toBe("Qwen/Qwen-Image");
    expect(firstBody).not.toHaveProperty("image");
    expect(secondBody.model).toBe("Qwen/Qwen-Image-Edit-2509");
    expect(secondBody.image).toContain("data:image/png;base64,");
  });
