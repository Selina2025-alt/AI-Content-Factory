  ...
  return (
    ...
    >
      ...
      {showImageSkills ? (
        <>
          <div className="settings-selection-group">
            <p className="settings-selection-group__label">图片模型</p>
            <label className="settings-model-select">
              <select
                aria-label={`${props.platform} 图片模型`}
                value={props.selectedImageModel}
                onChange={(event) => props.onSelectImageModel?.(event.target.value)}
              >
                {(props.availableImageModels ?? []).map((model) => (
