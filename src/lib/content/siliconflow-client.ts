...
const DEFAULT_IMAGE_FALLBACK_MODELS = SILICONFLOW_IMAGE_MODEL_OPTIONS.map(
  (option) => option.id
);
...
function parseImageFallbackModels() {
  ...
  const normalized = raw
    .split(",")
    .map((model) => normalizeSiliconFlowImageModel(model))
    .filter(Boolean);
...
export function getSiliconFlowImageConfig(
  preferredModel?: string
): SiliconFlowImageConfig | null {
  const apiKey = process.env.SILICONFLOW_API_KEY?.trim();
  const model = normalizeSiliconFlowImageModel(
    preferredModel ??
...
function supportsReferenceImage(model: string) {
  return /image-edit/i.test(model);
}
...
function shouldTryNextImageModel(status: number, bodyText: string) {
  ...
  if (
    status === 400 &&
    /does not support image|unsupported image|invalid image input|unknown parameter.*image|invalid parameter.*image/i.test(
      bodyText
    )
  ) {
    return true;
...
function resolveImageModelCandidates(input: {
  ...
  const deduped = new Set<string>([input.configuredModel, ...parseImageFallbackModels()]);
  const orderedModels = [...deduped];

  if (!input.hasInputImage) {
    return orderedModels;
  }

  const configuredModel = orderedModels[0];
  const tail = orderedModels.slice(1);
  const imageEditModels = tail.filter((model) => supportsReferenceImage(model));
  const textToImageModels = tail.filter((model) => !supportsReferenceImage(model));

  return [configuredModel, ...imageEditModels, ...textToImageModels];
}
...
export async function createSiliconFlowImageGeneration(
  ...
  const config = getSiliconFlowImageConfig(input.model);
  ...
  modelLoop: for (const model of modelCandidates) {
    ...
    for (let attempt = 1; attempt <= MAX_BUSY_RETRIES; attempt += 1) {
      ...
      try {
        ...
        if (input.image && supportsReferenceImage(model)) {
